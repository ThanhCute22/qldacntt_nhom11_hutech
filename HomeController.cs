...
using WebsiteBanHang.Models.QuanLyLienHe;
...
namespace WebsiteBanHang.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> LienHe(LienHeViewModel model)
        {
            ...
            {
                return Json(new { success = false, message = "Vui lòng kiểm tra lại thông tin nhập." });
            }

            try
            {
                // Tạo nội dung email
                var emailContent = $@"<h2>Thông Tin Liên Hệ Từ Khách Hàng</h2>
                ...
                await _emailService.GuiEmailAsync(
                    emailNguoiNhan: "phatdet2312@gmail.com",
                    tieuDe: "Tin Nhắn Liên Hệ Từ Website TechTrend",
                    noiDungHtml: emailContent
                );

                // Gửi email xác nhận đến khách hàng
                var confirmContent = $@"<h2>Cảm ơn bạn đã liên hệ với TechTrend!</h2>
                              <p>Xin chào {model.HoTen},</p>
                              ...
                              <p><strong>Số Điện Thoại:</strong> {model.SoDienThoai}</p>
                              <p><strong>Nội Dung:</strong> {model.NoiDung}</p>
                              <p>Trân trọng,<br>Đội ngũ TechTrend</p>";

                await _emailService.GuiEmailAsync(
                    emailNguoiNhan: model.Email,
                    tieuDe: "Xác Nhận Liên Hệ - TechTrend",
                    noiDungHtml: confirmContent
                );
            ...
            catch (Exception ex)
            {
                ...
                return Json(new { success = false, message = "Có lỗi xảy ra khi gửi tin nhắn. Vui lòng thử lại." });
            }
        }
        ...
        public async Task<IActionResult> SanPhams(string tuKhoaTimKiem = "", string chonDanhMuc = "", int? chonKho = null, string sapXepGia = "macDinh")
        {
            ...
            //Tính điểm khớp cho từng sản phẩm bằng hàm TinhSoTuKhop
