...
namespace WebsiteBanHang.Areas.Admin.Controllers
{
    ...
    public class QuanLyNguoiDungController : Controller
    {
        ...
        public async Task<IActionResult> PhanQuyen(string userId, List<string> selectedRoles)
        {
            ...
            // Nếu là siêu admin, đảm bảo vai trò "Admin" luôn được giữ
            if (user.IsSuperAdmin == true)
            {
                if (selectedRoles == null || !selectedRoles.Contains("Admin"))
                {
                    return Json(new { success = false, message = "Siêu admin không thể bỏ vai trò Admin của chính mình." });
                }
            }
        ...
        public async Task<IActionResult> ChangeStatus(string userId, bool status)
        {
            ...
            if (user.IsSuperAdmin && !status) // Ngăn khóa siêu admin
                return Json(new { success = false, message = "Không thể khóa tài khoản siêu admin." });
        ...
        public async Task<IActionResult> Display(string userId)
        {
            ...
            var model = new UserDetailsViewModel
            {
                ...
                IsSuperAdmin = user.IsSuperAdmin // Gán giá trị IsSuperAdmin
            };
            return View(model);
        }
